...
наборах. Для этого они занумеровали все цвета случайными числами от 0 до 108. На этом их энтузиазм иссяк, поэтому вам
...
у Ани и номера цветов кубиков, которые есть только у Бори. Для каждого из множеств выведите сначала количество элементов
...
share_box = Anya.intersection(Borya)
print(len(share_box))
print(*sorted([int(i) for i in list(share_box)]))

Anya_box = Anya.difference(Borya)
print(len(Anya_box))
print(*sorted([int(i) for i in list(Anya_box)]))

Borya_box = Borya.difference(Anya)
print(len(Borya_box))
print(*sorted([int(i) for i in list(Borya_box)]))
